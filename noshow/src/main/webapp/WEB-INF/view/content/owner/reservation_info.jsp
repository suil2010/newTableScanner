...
<script>
	...
	var curTime = curDate.getFullYear() + "-" + (curDate.getMonth() + 1) + "-" + curDate.getDate() + " " + curDate.getHours() + ":"
						...
						<td><c:choose>
								<c:when test="${reservation.resStartTime >= sysDate }">
									...
									<form action="${initParam.rootPath}/cancelReservation.do" method="post">
										<input type="submit" value="직권 예약취소" onclick="return confirm('정말 예약을 취소하시겠습니까?')"> <input type="hidden" name="resNum"
											value="${reservation.resNum }"
										>
										<sec:csrfInput />
									</form>
								</c:when>
