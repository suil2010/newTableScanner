...
/* 	function registReview() {
	...
	function changeResPeople() {
		...
		$.ajax({
			...
			"type":"get",
			"data":{"resPeople":$("#resPeople").val(),
				"businessId" : "${requestScope.restaurant.businessId }"},
			"success":function(totalPrice){
				alert("인원 변경->재계산 성공!");
				var txt = "총 예약 금액 : "
							+"<span style='color: red; float: center; font-size: 30px'><b>"
							+ totalPrice +"</b></span>";
...
<div class="container">
		...
		<div id="orderDiv">
			<c:choose>
				<c:when test="${requestScope.restaurant.bookmarkCheck == 1 }">
					<button id="deleteBookmarkBtn" class="btn btn-default btn-sm"
						name="deleteBookmartBtn">
						<span class="glyphicon glyphicon-heart"></span> 즐겨찾기 삭제
					</button>
				</c:when>
				<c:otherwise>
					<button id="bookmarkBtn" class="btn btn-default btn-sm"
						name="bookmartBtn">
	...
	<div class="row">
		<div class="col-sm-8">
			<ul class="nav nav-tabs" role="tablist" id="myTab">
				<li class="active"><a href="#menu" aria-controls="menu"
					role="tab" data-toggle="tab">메뉴</a></li>
				<li><a href="#review" aria-controls="review" role="tab"
					data-toggle="tab">리뷰</a></li>
				<li><a href="#question" aria-controls="question" role="tab"
					data-toggle="tab">문의</a></li>
				<li><a href="#table" aria-controls="table" role="tab"
					data-toggle="tab">테이블정보</a></li>
			</ul>
			...
			<div class="tab-content">
				...
				<div role="tabpanel" class="tab-pane" id="review"
					...
					<c:if test="${requestScope.restaurant.reservationCheck > 0 }">
						...
						<div class="col-sm-10">
							<input type="file" name="reviewPicture" id="reviewPicture" placeholder="리뷰 사진">
							<textarea class="form-control" rows="3" style="resize: none;"
								name="reviewText" id="reviewText" required></textarea>
						...
						<c:choose>
							...
							<c:otherwise>
								...
								<div class="col-md-6" style="margin-top: 10px;">
									<div style="margin-right: 10px; background: #fff; width: 100%; height: 100%; padding: 10px;" class="col-md-12">
									<span class="label col-sm-9" style="color: #000; font-size: 14px;">작성자 : ${review.memberId}</span>
									<img src="${initParam.rootPath }/reviewPicture/${review.reviewImg}" width="100px" height="100" class="col-sm-3"> 
									<span class="label col-sm-9" style="color: #000; font-size: 18px;">평점 : ${review.reviewGrade}</span> 
									<span class="label col-sm-9" style="color: #000; font-size: 14px;">내용 : ${review.reviewText}</span> 
									<span class="label col-sm-9" style="color: #000; font-size: 14px;">작성 시간 : ${review.reviewTime}</span>
								</div>
							</div>
						</c:forEach>
							</c:otherwise>
				...
				<div role="tabpanel" class="tab-pane" id="question"
					...
					<form action="#" method="post">
						...
						<div class="col-sm-2">
							<button style="width: 100%; height: 74px;"
								class="btn btn-default">문의작성</button>
						</div>
					</form>
				</div>
		...
		<!-- 주문표 -->
		<form name="resForm" action="${initParam.rootPath}/payment.do"
			onsubmit="return formCheck()" class="col-sm-4" method="post">
			<div class="panel panel-default">
				<div class="panel-heading" id="orderDiv">
					<h3 class="panel-title">주문표</h3>
				</div>
				<div class="panel-body">
					<c:set scope="page" var="current"
						value="<%=new Date(System.currentTimeMillis())%>" />
					<div class="form-group">
						<label>예약 식당</label> <input type="text" class="form-control"
							name="restaurantName" value="${requestScope.restaurant.rtName }"
							readonly>
					</div>
					<input type="hidden" name="businessId"
						value="${requestScope.restaurant.businessId }">
						...
						<c:choose>
							<c:when test="${requestScope.resDate == null }">
								<input type="date" class="form-control" id="resDate"
									name="resDate" min="${current }" max="2019-12-31"
									placeholder="예약희망 날짜를 입력하세용" required>
							</c:when>
							<c:otherwise>
								<input type="date" value="${requestScope.resDate }" id="resDate"
									class="form-control" name="resDate" min="${current }"
									max="2019-12-31" placeholder="예약희망 날짜를 입력하세용" required>
							</c:otherwise>
						</c:choose>
					</div>
					<div class="form-group">
						...
						<c:choose>
							<c:when test="${requestScope.resTime == null }">
								<input class="form-control timepicker" name="resStartTime"
									id="resStartTime" placeholder="몇시에 오실건가요?" required>
							</c:when>
							<c:otherwise>
								<input class="form-control timepicker" name="resStartTime"
									id="resStartTime" value="${requestScope.resTime }" required>
							</c:otherwise>
						</c:choose>
					</div>
					<div class="form-group">
						...
						<c:choose>
							<c:when test="${empty requestScope.resPeople}">
								<input type="number" class="form-control" name="resPeople" id="resPeople"
									placeholder="몇명이오는지알려줭" min="1" required>
							</c:when>
							<c:otherwise>
								<input type="number" value="${requestScope.resPeople }"
									class="form-control" name="resPeople" id="resPeople" min="1" required>
							</c:otherwise>
						</c:choose>
					</div>
					<div id="payment">
						<div class="col-sm-12">
							<label class="radio-inline"><input type="radio"
								name="resPayStatement" value="card">카드결제</label> <label
								class="radio-inline"><input type="radio"
								name="resPayStatement" value="vbank">무통장입금 </label>
						</div>
						<div class="col-sm-12">
							<label class="radio-inline"><input type="radio"
								name="resPayStatement" value="trans">실시간계좌이체 </label> <label
								class="radio-inline"><input type="radio"
								name="resPayStatement" value="phone">휴대폰소액결제 </label>
						</div>
					</div>
					...
					<div class="form-group" id="selectTableDiv"
						...
						<c:choose>
							...
							<c:otherwise>
								<c:forEach items="${requestScope.restaurant.usableTable }"
									var="tables" varStatus="cnt">
									<c:if test="${cnt.index % 2 == 0 }">
										<br>
									</c:if>
									<label>테이블 번호 : ${tables.tableNum }, 최대인원 :
										${tables.tablePeople } <input type="checkbox" name="tableList"
					...
					<div class="form-group" id="totalPriceDiv">
						총 예약 금액 : 
						<span style="color: red; float: center; font-size: 30px"><b>
						<fmt:formatNumber value="${requestScope.restaurant.rtDeposit * requestScope.resPeople }" pattern="#,### 원"/></b></span>
						<sec:csrfInput />
					</div>
